...
class BoardGameController: UIViewController {
    ...
    private func getCardsBy(modelData: [Card]) -> [UIView] {
        ...
        for (index, modelCard) in modelData.enumerated() {
            ...
            let cardOne = cardViewFactory.get(modelCard.type, withSize: cardSize, andColor: modelCard.color, maxBoardX: cardMaxXCoordinate, maxBoardY: cardMaxYCoordinate)
            cardOne.tag = index
            cardViews.append(cardOne)
            // добавляем второй экземпляр карты
            let cardTwo = cardViewFactory.get(modelCard.type, withSize: cardSize, andColor: modelCard.color, maxBoardX: cardMaxXCoordinate, maxBoardY: cardMaxYCoordinate)
            cardTwo.tag = index
